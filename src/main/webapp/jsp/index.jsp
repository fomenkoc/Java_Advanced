			...
			<c:when test="${mode == 'PARTICIPANT_EDIT' || mode == 'PARTICIPANT_CREATE'}">
				...
				<input type="hidden" value="${participant.id}" class="form-control" id="id" name="id">
					...
					<div class="form-group">
						<label for="participantName">Name:</label> <input type="text"
					...
					<div class="form-group">
						<label for="participantLevel">Level:</label> 
						<select class="form-control" id="level" name="level">
						<c:forEach var="level" items="${levels}">
						<option value="${level}" <c:choose><c:when test="${participant.level == level}">selected</c:when></c:choose>>${level}</option>
						</c:forEach>
						</select>
					</div>
					
					<div class="form-group">
						<label for="participantPrimarySkill">Primary Skill:</label> <input type="text"
